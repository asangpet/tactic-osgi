...
public class AnalysisGraph extends InstanceGraph {
	...
	ParametricDensity getParamFit(DiscreteProbDensity pdf, boolean nofit) {
		...
		if (pdf.getRaw()!=null)
			return matlab.getGevParamFit(pdf,pdf.getRaw());
		else
	...
	ParametricDensity getParamFitShape(DiscreteProbDensity pdf,double shape, boolean nofit) {
		if (nofit) return new ParametricDensity(pdf, new double[] {1,1,1});
		if (pdf.getRaw() != null)
			return matlab.getGevParamFit(pdf,pdf.getRaw(),shape);
		else
		...
		if (childs.size() == 0) {
			...
			if (node.serverResponse != null) {
				if (node.model == null) {
					...
					node.model = new NodeModel(fitResult.getPdf(), fitResult.getParam(), node.getServerResponse().getRawCount());
					...
					node.modelpdf = fitResult.getPdf();
			...
			if (unknownLink.size() == 0) {
				...
				if (node.serverResponse != null) {
					...
					if (distRespPdf == null) {
						...
						if (shapeshift) {
							if (inputGev.getParam()[0] <= 0) inputGev.getParam()[0] = newFit.getParam()[0];
							if (inputGev.getParam()[0] <= 0) inputGev.getParam()[0] = 1;

							transfer = new TransferFunction(new double[] {newFit.getParam()[0]/inputGev.getParam()[0], newFit.getParam()[1]/inputGev.getParam()[1], newFit.getParam()[2]-inputGev.getParam()[2]},
									modelTransfer);
						} else {
							transfer = new TransferFunction(new double[] { newFit.getParam()[0], newFit.getParam()[1]/inputGev.getParam()[1], newFit.getParam()[2]-inputGev.getParam()[2] },
									modelTransfer);
						}
						...
						for (ParametricDensity it:distResp) {
							...
							ParametricDensity inputGev = getParamFit(it.getPdf(), nonparamPredict);
				...
				if (node.analysisResponse == null) {
					if (distRespPdf == null) {
						...
						if (node.model != null) {
							...
							if (nonparamPredict) {
								predictPdf = matlab.filter(compositeRespPdf, node.model.transfer.getNonparamPdf()).ensurePositive().cutoff(node.model.cutoff);
							} else if (shapeshift) {
								predictPdf = getPdf(node.model.transfer.param[0]*inputGev.param[0],
										node.model.transfer.param[1]*inputGev.param[1],
										node.model.transfer.param[2]+inputGev.param[2]);
						...
						if (node.model != null) {
							...
							for (ParametricDensity it:distResp) {
								...
								if (nonparamPredict) {
									dPdf.add(matlab.filter(it.getPdf(), node.model.transfer.getNonparamPdf()));
								} else {
			...
			if (distRespPdf == null) {
				...
				if (node.model != null) {
					...
					if (nonparamPredict) {
						if (node.transferEdited) {
							predictPdf = matlab.filter(compositeRespPdf, node.model.transfer.editedNonparamPdf).ensurePositive().cutoff(node.model.cutoff);
						} else {
							predictPdf = matlab.filter(compositeRespPdf, node.model.transfer.nonparamPdf).ensurePositive().cutoff(node.model.cutoff);
						}
					}
				...
				if (node.model!=null) {
					...
					for (ParametricDensity it:distResp) {
						...
						if (nonparamPredict) {
							if (node.transferEdited) {
								dPdf.add(matlab.filter(it.pdf, node.model.transfer.editedNonparamPdf));
							} else {
								dPdf.add(matlab.filter(it.pdf, node.model.transfer.nonparamPdf));
							}
						} else {
