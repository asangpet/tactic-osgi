...
public class DiscreteProbDensity {
	...
	public static void main(String[] args) {
		...
		DiscreteProbDensity xdpd = new DiscreteProbDensity(20, 0, 100, 0);
		DiscreteProbDensity ydpd = new DiscreteProbDensity(20, 0, 100, 0);
		NormalDistribution normal = new NormalDistribution(40, 10);
		NormalDistribution ynormal = new NormalDistribution(60, 15);
		for (int i=0;i<1000;i++) {
			double norm = normal.sample();
			if (norm < 0) norm = 0;
			if (norm > 100) norm = 100;
			xdpd.add(norm);

			double ynorm = ynormal.sample();
			if (ynorm < 0) ynorm = 0;
			if (ynorm > 100) ynorm = 100;
			ydpd.add(ynorm);
		}
		xdpd = xdpd.normalize();
		ydpd = ydpd.normalize();
