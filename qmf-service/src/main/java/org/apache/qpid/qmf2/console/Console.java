...
public final class Console implements MessageListener, AgentProxy
{
    ...
    /**
     * If a name is supplied, it must be unique across all Consoles attached to the AMQP bus under the given domain.
     * If no name is supplied, a unique name will be synthesized in the format: "qmfc-<hostname>.<pid>"
     */
    ...
    private void broadcastAgentLocate()
    {
        ...
        {
            log.info("JMSException {} caught in broadcastAgentLocate()", jmse.getMessage());
        }
    }
    ...
    private void handleAgentExpiry()
    {
        ...
        while (i.hasNext())
        {
            ...
            if (diff > _agentTimeout)
            {
                ...
                agent.deactivate();
                i.remove();
                log.info("Agent {} has expired", agent.getName());
...
//System.out.println("contentType = " + contentType);
            ...
            { // This block handles Agent lifecycle information (discover, register, delete)
                ...
                { // This block handles Agents that have previously been registered
                    ...
                    if (agent.getEpoch() != originalEpoch)
                    {
                        ...
                        log.info("Agent {} has been restarted", agentName);
                        if (_discoverAgents && (_agentQuery == null || _agentQuery.evaluate(agent)))
                        {
                    ...
                    { // Otherwise just send a heartbeat notification
                        log.info("Agent {} heartbeat", agent.getName());
                        if (_discoverAgents && (_agentQuery == null || _agentQuery.evaluate(agent)))
                        {
                ...
                { // This block handles Agents that haven't already been registered
                    ...
                    log.info("Adding Agent {}", agentName);
                    ...
                    if (agent.getVendor().equals("apache.org") && agent.getProduct().equals("qpidd"))
                    {
                        log.info("Recording {} as _brokerAgentName", agentName);
            ...
            if (!_agents.containsKey(agentName))
            {
                log.info("Ignoring Event from unregistered Agent {}", agentName);
                return;
            }
            ...
            {
                log.info("{} has disabled Event reception, ignoring Event", agentName);
                return;
            }
            ...
            if (opcode.equals("_method_response") || opcode.equals("_exception"))
            {
                ...
                {
                    log.info("onMessage() Received Method Response message in incorrect format");
                }
            }
            ...
            if (opcode.equals("_query_response") && content.equals("_data"))
            {
                ...
                {
                    log.info("onMessage() Received Query Response message in incorrect format");
                }
            }
            ...
            if (opcode.equals("_subscribe_response"))
            {
                ...
                {
                    log.info("onMessage() Received Subscribe Response message in incorrect format");
                }
            }
            ...
            if (opcode.equals("_data_indication") && content.equals("_data"))
            {
                ...
                {
                    log.info("onMessage() Received Subscribe Indication message in incorrect format");
                }
            }
            ...
            { // There are differences in the type of message sent by Qpid 0.8 and 0.10 onwards.
                ...
                { // 0.10 and above broker passes Events as amqp/list encoded as BytesMessage (needs decoding into a List)
                    List<Map> list = AMQPMessage.getList(message);
                    for (Map m : list)
                    {
                ...
                {
                    log.info("onMessage() Received Event message in incorrect format");
                }
            }
        }
        catch (JMSException jmse)
        {
            log.info("JMSException {} caught in onMessage()", jmse.getMessage());
        }
    } // end of onMessage() 
    ...
    private List<SchemaClass> getSchema(final List<SchemaClassId> classes, final Agent agent)
    {
        ...
        {
            agent.setSchema(classId, Collections.<SchemaClass>emptyList()); // Clear Agent's schema value for this classId
            ...
            synchronized(this)
            {
                ...
                if (replyHandle == null)
                {
                    ...
                    else
                    {
                        ...
                        results = new ArrayList<QmfConsoleData>(mapResults.size());
    ...
    /**
     * Releases the specified Agent instance. Once called, the console application should not reference this instance again.
     * <p>
     ...
     * @param domain the QMF "domain". A QMF address is composed of two parts - an optional domain string, and a mandatory
     *        name string <pre>"qmf.&lt;domain-string&gt;.direct/&lt;name-string&gt;"</pre>
     *        The domain string is used to construct the name of the AMQP exchange to which the component's     
     *        name string will be bound. If not supplied, the value of the domain defaults to "default". Both
     *        Agents and Components must belong to the same domain in order to communicate.
     * @param notifier this may be either a QMF2 API Notifier object OR a QMFEventListener.
     ...
     * </pre>
     */
    public Console(final String name, final String domain,
                   final QmfCallback notifier, final String options) throws QmfException
    {
        ...
        if (_name == null)
        {
            ...
            if (split.length == 2)
            {
                _name = "qmfc-" + split[1] + "." + split[0];
            }
            else
            {
                _name = "qmfc-" + UUID.randomUUID();
            }
        }

        _domain = (domain == null) ? "default" : domain;
        _address = "qmf." + _domain + ".direct" + "/" + _name;
     ...
     * finer control over the Console's request and reply queues, such as an explicit name, non-default size or durability.
     *
     * @param conn a javax.jms.Connection
    ...
    public void addConnection(final Connection conn, final String addressOptions) throws QmfException
    {
        ...
        try
        {
            ...
            synchronized(this)
            {
                ...
                // Broadcast an Agent Locate message 
                ...
                while (!_agentAvailable)
                {
                    ...
                    {
                        log.info("Broker Agent not found");
        ...
        catch (JMSException jmse)
        {
            ...
            log.info("JMSException {} caught in addConnection()", jmse.getMessage());
    ...
    /**
     * Return the the named Agent, if known.
     *
     * @param agentName the name of the Agent to be returned.
     * <p>
     * "broker" or "qpidd" may be used as synonyms for the broker Agent name and the method will try to match
    ...
    public List<SchemaClassId> getClasses(final Agent agent)
    {
        ...
        try
        {
            ...
            synchronized(this) 
            {
                ...
                if (response == null)
                {
                    log.info("No response received in getClasses()");
                    return Collections.emptyList();
                }
...
//new SchemaClassId(content).listValues();
        ...
        {
            log.info("JMSException {} caught in getClasses()", jmse.getMessage());
        }
...
//System.out.println("getSchema for agent " + agentName);
        ...
        try
        {
            ...
            synchronized(this)
            {
                ...
                if (response == null)
                {
                    log.info("No response received in getSchema()");
                    return Collections.emptyList();
                }
...
//schema.listValues();
        ...
        {
            log.info("JMSException {} caught in getSchema()", jmse.getMessage());
        }
                                                ...
                                                final String consoleHandle, final String options) throws QmfException
    {
        ...
        try
        {
            ...
            { // If this is an synchronous request get the response
                ...
                {
                    log.info("No response received in createSubscription()");
        ...
        {
            log.info("JMSException {} caught in createSubscription()", jmse.getMessage());
            throw new QmfException(jmse.getMessage());
        }
    ...
    public SubscribeParams refreshSubscription(String subscriptionId, final String options) throws QmfException
    {
        ...
        try
        {
            ...
            synchronized(this)
            {
                ...
                { // If this is an synchronous request get the response
                    ...
                    {
                        subscription.cancel();
                        log.info("No response received in refreshSubscription()");
        ...
        {
            log.info("JMSException {} caught in refreshSubscription()", jmse.getMessage());
            throw new QmfException(jmse.getMessage());
        }
    ...
    public void cancelSubscription(final String subscriptionId) throws QmfException
    {
        ...
        {
            log.info("JMSException {} caught in cancelSubscription()", jmse.getMessage());
        }
    }
}
